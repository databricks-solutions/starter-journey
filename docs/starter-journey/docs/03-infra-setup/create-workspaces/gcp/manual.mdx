---
sidebar_label: Manual
description: Create a Databricks workspace on GCP manually through the account console.
---

import { StepGuide, Step } from '@site/src/components/StepGuide';

# GCP manual workspace creation

> **You'll create** a Databricks workspace on GCP in ~10 min.
>
> **Prereqs:** [GCP project](/docs/02-before-you-start/cloud-tenant-ready), [Databricks account console](/docs/02-before-you-start/foundations/account-console)

## The big picture

A running Databricks workspace in your GCP project, created from the Databricks account console. The wizard handles the service account, GCS storage, and workspace configuration for you.

## Prerequisites

- A GCP project with Owner permissions.
- A Databricks account with account-admin privileges.
- The GCP project the workspace belongs to (for example, the dev workspace in the dev project).

## Steps

<StepGuide
  description="Create each workspace from the account console, following the video. Deploy each one into the matching GCP project."
  minutes={10}
  referenceHref="https://docs.gcp.databricks.com/en/admin/account-settings-gcp/workspaces.html"
  referenceLabel="GCP workspace administration (Databricks docs)">

<Step>

Create the development workspace by following the video below. As you deploy, pick the right GCP project for the environment.

:::warning
Deploy each workspace into the matching GCP project. A dev workspace in a production project causes confusion and makes cleanup harder.
:::

<iframe
  width="560"
  height="315"
  src="https://www.youtube.com/embed/4XMMXBbE-tg"
></iframe>

</Step>

<Step>

Run the same process for each additional workspace, picking the matching GCP project each time.

</Step>

</StepGuide>

## Verify

<StepGuide eyebrow="Verify" idPrefix="verify">

<Step>

Log in to the [Databricks account console](https://accounts.gcp.databricks.com/).

</Step>

<Step>

Open **Workspaces** and confirm the new workspace shows **Running**.

</Step>

<Step>

Click the workspace URL and confirm the landing page loads.

</Step>

</StepGuide>

## Where people trip

<details>
<summary>Workspace stuck in provisioning</summary>

Check the GCP console for API enablement issues. The workspace needs several GCP APIs enabled on the project: Compute Engine, Cloud Storage, and IAM.
</details>

<details>
<summary>Permission errors during creation</summary>

Confirm the authenticated account has Owner on the target GCP project. Viewer and Editor are not enough for the initial workspace creation.
</details>

## Next

- **Do next:** [GCP Terraform deployment](/docs/03-infra-setup/create-workspaces/gcp/terraform)
- **Learn why:** [Unity Catalog foundations](/docs/02-before-you-start/foundations/unity-catalog)
- **Reference:** [GCP workspace administration](https://docs.gcp.databricks.com/en/admin/account-settings-gcp/workspaces.html)
