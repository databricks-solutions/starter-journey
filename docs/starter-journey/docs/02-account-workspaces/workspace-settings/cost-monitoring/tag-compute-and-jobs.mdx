...
>
> **Prereqs:** [Account and workspaces](/docs/02-account-workspaces)
...
- **Account admin** for workspace-level tags through the Account API and for tag-aware budgets on [Budget alerts](/docs/02-account-workspaces/workspace-settings/cost-monitoring/budget-alerts).
...
<StepGuide
  description="Create a serverless usage policy so serverless workloads inherit your team or project tags."
  eyebrow="Serverless usage policy"
  referenceHref="https://docs.databricks.com/aws/en/admin/account-settings/usage-detail-tags"
  referenceLabel="Use tags to attribute and track usage">

<Step>

Go to Avatar → **Settings** → **Compute**.
...
Click **Create**, name the policy, and add tag pairs (for example `team:data-engineering`).

</Step>

<Step>

Open **Permissions** → **Grant access** and assign **User** or **Manager** roles.
...
Tag a cluster `test_tag:verification`, run some work, wait **2 to 4 hours**, then filter **`system.billing.usage`** on that map key.

</Step>

<Step>

Assign yourself a serverless usage policy, run serverless work, then confirm **`budget_policy_id`** is populated.

</Step>

<Step>

In **AWS Cost Explorer** (or its equivalent), confirm the propagated tags when classic compute backs the bill.
...
- **Do next:** [Budget alerts](/docs/02-account-workspaces/workspace-settings/cost-monitoring/budget-alerts)
