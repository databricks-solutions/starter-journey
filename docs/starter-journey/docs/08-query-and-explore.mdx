---
sidebar_label: 8. Query and Explore
description: Run interactive SQL against your lakehouse data using the Databricks SQL Editor and a serverless warehouse.
---
import StarterJourneyProgress from '@site/src/components/StarterJourneyProgress';
import { StepGuide, Step } from '@site/src/components/StepGuide';

# 8. Query and explore

> **You'll run** interactive SQL against your lakehouse tables in the Databricks SQL Editor in ~5 min.
>
> **Prereqs:** [Automation & Orchestration](/docs/12-orchestration/)

## The big picture

Once a pipeline writes data, someone has to look at it. You check that the transforms did what you meant, eyeball a few distributions, and answer the one-off question your stakeholder just asked. The SQL Editor plus a serverless warehouse gives you a place to do that. No cluster to size, no waiting on startup. You open the editor and run a query against any table your Unity Catalog grants let you see.

<StarterJourneyProgress currentLevel={5}/>

## Run your first query

<StepGuide
  description="Open the SQL Editor, pick the serverless warehouse, set your context, and run a query."
  minutes={5}>

<Step>

Open the SQL Editor from the left nav. Every workspace ships with a **Starter Serverless Warehouse**, a managed endpoint built for interactive SQL, so you do not have to create compute first.

</Step>

<Step>

Pick the Starter Serverless Warehouse in the warehouse dropdown if it is not already selected.

</Step>

<Step>

Set the catalog and schema in the context bar so you can use plain table names instead of `catalog.schema.table` everywhere.

</Step>

<Step>

Type a query and run it. The warehouse spins up in a few seconds and returns results in the grid below.

</Step>

</StepGuide>

:::tip
Drag a table from the schema browser straight into the editor and it drops in the fully qualified name for you.
:::

### Video walkthrough

<iframe
  width="560"
  height="315"
  src="https://www.youtube.com/embed/0XWf1V40SdI"
></iframe>

## Where people trip

- **Reaching for a pro warehouse out of habit.** Serverless starts faster and scales on its own, so use it for interactive work unless your admin set up a specific warehouse for your team.
- **Querying with no context set.** If you skip the catalog and schema in the context bar, every query needs the full three-part name (`catalog.schema.table`). Set it once and the editor fills in the rest.

## Next

| Page | What you'll do |
|---|---|
| [9. Unified analytics](/docs/09-unified-analytics/) | Build metric views, dashboards, Genie Spaces, and apps on top of your lakehouse data. |
| [10. Predictive analytics](/docs/10-predictive-analytics/) | Register models in Unity Catalog and run batch inference at scale. |
| [11. Agents](/docs/11-agents/) | Build and deploy AI agents grounded in your lakehouse data. |
