...
```sql
ALTER TABLE main.lab_dac.employees ALTER COLUMN salary      SET TAGS ('pii' = 'salary');
ALTER TABLE main.lab_dac.employees ALTER COLUMN ssn         SET TAGS ('pii' = 'ssn');
ALTER TABLE main.lab_dac.employees ALTER COLUMN email       SET TAGS ('pii' = 'email');
ALTER TABLE main.lab_dac.employees ALTER COLUMN card_number SET TAGS ('pii' = 'card_number');
ALTER TABLE main.lab_dac.employees ALTER COLUMN phone       SET TAGS ('pii' = 'phone');
ALTER TABLE main.lab_dac.employees ALTER COLUMN region      SET TAGS ('pii' = 'region');
```
...
**Checkpoint:** `DESCRIBE TABLE EXTENDED main.lab_dac.employees` shows a `pii` tag on each of the six columns.
...
-- CREATE OR REPLACE POLICY policy_ssn
-- ON SCHEMA main.lab_dac
-- COLUMN MASK main.lab_dac.hash_ssn
-- TO `account users` EXCEPT hr_admin, compliance
-- FOR TABLES
-- MATCH COLUMNS has_tag_value('pii', 'ssn') AS ssn
-- ON COLUMN ssn;
...
-- CREATE OR REPLACE POLICY policy_email
-- ON SCHEMA main.lab_dac
-- COLUMN MASK main.lab_dac.mask_email
-- TO `account users` EXCEPT data_owners
-- FOR TABLES
-- MATCH COLUMNS has_tag_value('pii', 'email') AS email
-- ON COLUMN email;
...
**Checkpoint:** Run `SELECT * FROM main.lab_dac.employees` as a user who isn't in any of the groups above. You should see:
...
```sql
CREATE OR REPLACE FUNCTION main.lab_dac.filter_region(region STRING)
RETURNS BOOLEAN
RETURN
...
ROW FILTER main.lab_dac.filter_region
TO `account users` EXCEPT hr_admin, compliance
FOR TABLES
...
ALTER TABLE main.lab_dac.contractors ALTER COLUMN salary SET TAGS ('pii' = 'salary');
ALTER TABLE main.lab_dac.contractors ALTER COLUMN ssn    SET TAGS ('pii' = 'ssn');
ALTER TABLE main.lab_dac.contractors ALTER COLUMN email  SET TAGS ('pii' = 'email');
```

**Checkpoint:** `SELECT salary, ssn, email FROM main.lab_dac.contractors` as a non-privileged user returns `NULL`, `XXX-XX-XXXX`, and a 64-character hash. The policies were defined once, on the schema. Tagging is the only per-table step, and that's the point: a data owner tags a column, they don't have to know a masking policy exists.
...
```sql
-- Drop policies
DROP POLICY policy_salary ON SCHEMA main.lab_dac;
DROP POLICY policy_ssn    ON SCHEMA main.lab_dac;
DROP POLICY policy_email  ON SCHEMA main.lab_dac;
DROP POLICY policy_card   ON SCHEMA main.lab_dac;
DROP POLICY policy_phone  ON SCHEMA main.lab_dac;
DROP POLICY policy_region ON SCHEMA main.lab_dac;
...
DROP FUNCTION IF EXISTS main.lab_dac.filter_region;
