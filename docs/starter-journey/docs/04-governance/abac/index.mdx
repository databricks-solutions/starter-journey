...
>
> **Prereqs:** [5. Data Governance Strategy](/docs/04-governance/), [Unity Catalog foundations](/docs/01-introduction/foundations/unity-catalog), [Groups](/docs/04-governance/groups)
...
See the [hands-on lab](/docs/04-governance/abac/lab) for the full UDF and policy implementation for each column type, including both redaction and SHA-256 variants.
...
Everyone in `region_emea` sees only EMEA rows on any tagged table in `main.hr`. `hr_admin` is exempt and sees everything. See the [hands-on lab](/docs/04-governance/abac/lab) for a full worked example with three regions.
...
- [Hands-on lab](/docs/04-governance/abac/lab): apply ABAC tag-driven policies to an employees table with salary, SSN, email, and credit card columns, including SHA-256 hashing and row-level filtering by region (~55 min)

## Next

- **Do next:** [Hands-on lab](/docs/04-governance/abac/lab)
- **Learn why:** [Unity Catalog foundations](/docs/01-introduction/foundations/unity-catalog), [Data Governance Strategy](/docs/04-governance/)
