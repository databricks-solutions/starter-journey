...
|---|---|---|
| **Lakeflow Connect** | Ingest data from external sources into UC tables | [Access your data — ingestion pipelines](/docs/06-access-your-data/managed-connectors/create-ingestion-pipeline/) |
| **Spark Declarative Pipelines** | Transform raw data through bronze, silver, and gold layers | This section |
| **Lakeflow Jobs** | Orchestrate pipelines, notebooks, and tasks on a schedule | [Automation and orchestration](/docs/08-orchestration/workspace) |
...
- **[Workspace + Genie Code](/docs/07-build-first-pipeline/workspace-databricks-agent)** — Build a pipeline interactively using the Databricks Workspace and Genie Code.
- **[DABs](/docs/07-build-first-pipeline/dabs)** — Deploy a complete medallion pipeline as code using Databricks Asset Bundles.
