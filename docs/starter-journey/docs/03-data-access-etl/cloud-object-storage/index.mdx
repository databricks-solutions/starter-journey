---
sidebar_position: 0
sidebar_label: Cloud Object Storage
description: Connect Databricks to your cloud object storage (S3, ADLS, GCS) using Unity Catalog storage credentials and external locations.
---

# Cloud Object Storage

> **You'll understand** how Databricks connects to cloud object storage through Unity Catalog in ~5 min.
>
> **Prereqs:** [Data access and ETL](/docs/03-data-access-etl)

## The big picture

Most of your data probably already sits in cloud object storage: S3 buckets, ADLS containers, GCS buckets.
Databricks needs a governed way to read and write it.
Skip that, and teams paste credentials into notebooks or set them at the cluster level, which sidesteps Unity Catalog completely.
Then nobody can tell you who touched what.

Think of it as a key and a door.
The key proves who you are.
The door decides what you can open.
Two Unity Catalog objects play those roles.

![Storage credentials and external locations](/img/uc-cloud-object-idea.png)

The **storage credential** is the key. It authenticates Databricks compute to your cloud storage by wrapping a cloud identity:
- An IAM role on AWS and GCP.
- A managed identity on Azure.

The **external location** is the door. It points at one specific path and is bound to a credential:
- AWS: `s3://mybucket/mydepartment/mydataset/`
- Azure: `abfss://mycontainer@mystorageaccount.dfs.core.windows.net/mydataset/`
- GCP: `gs://mybucket/mydepartment/mydataset/`

The credential proves who you are.
The external location and its grants decide who reaches which path.

## How it works

1. A metastore admin creates a **storage credential** that references a cloud IAM identity.
2. The admin creates one or more **external locations** that point to specific paths and are bound to that credential.
3. Unity Catalog grants on the external location control which users and groups can read or write data at that path.
4. Any notebook, job, or SQL query that references the path goes through UC permission checks automatically.

## Create storage credentials and external locations

Pick your cloud provider and follow the step-by-step guide:

- **[AWS](/docs/03-data-access-etl/cloud-object-storage/aws)**: storage credential and external location for Amazon S3.
- **[Azure](/docs/03-data-access-etl/cloud-object-storage/azure)**: access connector, storage credential, and external location for ADLS Gen2.
- **[GCP](/docs/03-data-access-etl/cloud-object-storage/gcp)**: storage credential and external location for Google Cloud Storage.

## Next

- **Do next:** [AWS S3 storage setup](/docs/03-data-access-etl/cloud-object-storage/aws)
- **Learn why:** [Unity Catalog foundations](/docs/01-introduction/foundations/unity-catalog)
- **Reference:** [Connect to cloud object storage using Unity Catalog](https://docs.databricks.com/aws/en/connect/unity-catalog/cloud-storage/)
